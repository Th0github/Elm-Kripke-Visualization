...
testUpdateWorldInput =
    let
        (updatedModel, _) = update (UpdateWorldInput "1") initialModel
    in
...
testAddWorld =
    let
        modelWithWorldInput = { initialModel | worldInput = "1" }
        (updatedModel, _) = update AddWorld modelWithWorldInput
    in
...
testUpdateAgentInput =
    let
        (updatedModel, _) = update (UpdateAgentInput "Alice") initialModel
    in
...
testAddAgent =
    let
        modelWithAgentInput = { initialModel | agentInput = "Alice" }
        (updatedModel, _) = update AddAgent modelWithAgentInput
    in
